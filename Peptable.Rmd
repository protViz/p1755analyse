...
path <- "../output/jennifergarcia_20150923_firstBatchFromCRG.sqlite3"
my_mqdb <- src_sqlite(path)
...
annotations <- tbl_df(read.table("../output/filemappingFinal.txt",header=TRUE))
annotations$filename


annotations <- annotations %>% select(filename,displayname)
...
dd <- plotPepFrequencies(resMRMQuant, main="all peptide count")
dev.off()

par(mar=c(10,3,2,2))
matplot(t(dd),type="l",axes=T)
axis(1,at=1:ncol(dd),labels=colnames(dd),las=2,cex=0.7)
